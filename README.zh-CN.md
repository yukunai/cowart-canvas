...
- 深色专业剪辑工作台包含素材缩略图、右侧预览与检查器、真实片段画面缩略图、绿色原声波形和底部多轨时间线。
- 默认提供 V1-V3 三条视频轨；片段可上下拖到不同图层，继续拖出最上方会自动增加 V4-V8，空置的最高轨道会自动回收。
- AI 精剪会根据主要镜头切换和说话停顿选择完整内容段，并分散覆盖原片；目标时长只增删完整段，不会按固定秒数切碎或截断最后一段。
- 支持片段分割、删除、复制、恢复、拖动排序、撤销/重做和精确修改起止时间。
- 长时间线支持缩放与上下双滚动条，片段点击、拖动和播放头定位使用同一套编辑时间。
- AV1、4K 或长视频会在首次导入时生成可缓存的 H.264 低分辨率预览代理，提升播放和拖动流畅度；最终导出仍使用高清原片。
- 时间线包含至少三条视频轨，以及 A1 原声、A2 配音/音乐和 T1 字幕独立轨道。
