...
Cowart Canvas 是一个运行在本机浏览器里的改图画布。它的核心用途是：把 AI 图片中需要修改的位置直接圈出来、写出来、标出来，然后整理成 Codex 可以继续改图的任务说明，或者直接提交到图片生成/改图 API。
...
- 独立“文本”页面支持分类、多笔记管理、实时预览和导出 Markdown，数据保存在本机浏览器。
- 界面支持中文和英文切换。
- 图生视频支持 1 到 5 张参考图。
...
文本页支持分类、搜索当前分类、多篇内容管理、实时 Markdown 预览和导出 `.md` 文件。
